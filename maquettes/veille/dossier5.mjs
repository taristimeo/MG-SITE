...
import { VEILLE } from './dossier4-content.mjs';
import { CONCEPTS, RECO, META } from './dossier5-content.mjs';
const { chromium } = pw;
...
/* ── Concept : deux écrans larges ───────────────────────────── */
const cEcrans2 = (c, idx, folioR, titre) => P(`
  <div style="display:flex;align-items:baseline;gap:6mm"><span class="num">Concept ${c.num}</span><span class="wide" style="font-size:20pt">${esc(c.nom)}</span><span class="cond" style="margin-left:auto;font-size:6.4pt;color:var(--bone-faint)">${esc(titre)}</span></div>
  <div class="grid2" style="margin-top:4mm;gap:8mm">
    ${idx.map(i => `<div><img class="shot" src="${img(`${SHOTS}/${c.id}-d${i}.jpg`)}" alt=""><p class="cap"><b>${String(i + 1).padStart(2, '0')}</b> — ${esc(c.etats[i])}</p></div>`).join('')}
  </div>
  ${c.ecrans2Txt ? `<div class="grid2" style="margin-top:auto;gap:8mm">${c.ecrans2Txt.map(t => `<div><p class="lbl">${esc(t[0])}</p><p class="txt" style="font-size:8.6pt">${rich(t[1])}</p></div>`).join('')}</div>` : ''}`, `Concept ${c.num} — ${esc(c.nom)}`, folioR);

/* ── Concept : rangée de téléphones ─────────────────────────── */
const cPhones = (c) => P(`
  <div style="display:flex;align-items:baseline;gap:6mm"><span class="num">Concept ${c.num}</span><span class="wide" style="font-size:20pt">${esc(c.nom)}</span><span class="cond" style="margin-left:auto;font-size:6.4pt;color:var(--bone-faint)">iPhone — les six états</span></div>
  <p class="lede" style="max-width:170mm;font-size:9.6pt">${rich(c.mobileLede)}</p>
  <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:4.5mm;margin-top:4mm">
    ${c.mobile.map((m, i) => `<div><img class="phone" src="${img(`${SHOTS}/${c.id}-m${i}.jpg`)}" alt=""><p class="cap"><b>${String(i + 1).padStart(2, '0')}</b> — ${esc(m)}</p></div>`).join('')}
  </div>
  <div class="grid2" style="margin-top:auto;gap:12mm"><div><p class="lbl">Sur le téléphone</p><p class="txt" style="font-size:8.6pt">${rich(c.mobileTxt)}</p></div><div><p class="lbl">Ce qu'on mesure</p><p class="txt" style="font-size:8.6pt">${rich(c.mesure)}</p></div></div>`, `Concept ${c.num} — ${esc(c.nom)}`, 'iPhone I');

/* ── Concept : anatomie d'un écran iPhone ───────────────────── */
const cAnatomie = (c) => P(`
  <div style="display:flex;align-items:baseline;gap:6mm"><span class="num">Concept ${c.num}</span><span class="wide" style="font-size:20pt">${esc(c.nom)}</span><span class="cond" style="margin-left:auto;font-size:6.4pt;color:var(--bone-faint)">iPhone — anatomie &amp; gestes</span></div>
  <div style="display:grid;grid-template-columns:58mm 58mm 1fr;gap:9mm;margin-top:5mm;align-items:start">
    ${c.anatomie.map(a => `<div>
      <div style="position:relative"><img class="phone" src="${img(`${SHOTS}/${c.id}-m${a.state}.jpg`)}" alt="">
      ${a.callouts.map((k, i) => `<div style="position:absolute;left:${k.x}%;top:${k.y}%;width:5.2mm;height:5.2mm;border-radius:50%;background:var(--terra);color:#fff;font-family:var(--cond);font-size:6.4pt;display:flex;align-items:center;justify-content:center;transform:translate(-50%,-50%);box-shadow:0 0 0 1.2px #f6f3ec">${i + 1}</div>`).join('')}</div>
      <p class="cap">${esc(a.cap)}</p>
      <ol style="margin:2mm 0 0;padding-left:4.5mm;font-size:7.2pt;line-height:1.4;color:var(--bone-dim)">${a.callouts.map(k => `<li style="margin-bottom:0.8mm">${rich(k.t)}</li>`).join('')}</ol>
    </div>`).join('')}
    <div>
      <p class="lbl">Les gestes sur iPhone</p>
      <table class="tb"><tr><th style="width:30mm">Geste</th><th>Ce qui se passe</th></tr>${c.gestesMobile.map(g => `<tr><td>${esc(g[0])}</td><td>${rich(g[1])}</td></tr>`).join('')}</table>
    </div>
  </div>`, `Concept ${c.num} — ${esc(c.nom)}`, 'iPhone II');

/* ── Concept : Safari, performance, ce qui change ───────────── */
const cIphoneNotes = (c) => P(`
  <div style="display:flex;align-items:baseline;gap:6mm"><span class="num">Concept ${c.num}</span><span class="wide" style="font-size:20pt">${esc(c.nom)}</span><span class="cond" style="margin-left:auto;font-size:6.4pt;color:var(--bone-faint)">iPhone — Safari, performance, différences</span></div>
  <div class="grid3" style="margin-top:6mm;gap:9mm">
    ${c.iphone.map(col => `<div><p class="lbl">${esc(col.lbl)}</p><ul class="list">${col.items.map(i => `<li style="font-size:8.3pt">${rich(i)}</li>`).join('')}</ul></div>`).join('')}
  </div>
  <div style="margin-top:auto"><hr class="rule"><p class="txt" style="margin-top:3mm;font-size:8.6pt">${rich(c.iphoneBas)}</p></div>`, `Concept ${c.num} — ${esc(c.nom)}`, 'iPhone III');

/* ── Concept : technique seule ──────────────────────────────── */
const cTech = (c) => P(`
  <div style="display:flex;align-items:baseline;gap:6mm"><span class="num">Concept ${c.num}</span><span class="wide" style="font-size:20pt">${esc(c.nom)}</span><span class="cond" style="margin-left:auto;font-size:6.4pt;color:var(--bone-faint)">Mise en œuvre</span></div>
  <div class="grid2" style="margin-top:6mm;gap:12mm">
    <div><p class="lbl">Technique</p><ul class="list">${c.tech.map(i => `<li>${rich(i)}</li>`).join('')}</ul></div>
    <div style="display:flex;flex-direction:column;gap:6mm">
      <div><p class="lbl">Ce qu'il faut de vous</p><p class="txt">${rich(c.besoin)}</p></div>
      <div><p class="lbl">Risques &amp; parades</p><p class="txt">${rich(c.risques)}</p></div>
      ${c.mesure ? `<div><p class="lbl">Ce qu'on mesure</p><p class="txt">${rich(c.mesure)}</p></div>` : ''}
      <div style="display:flex;gap:10mm;margin-top:auto">${c.chiffres.map(k => `<div><div class="wide" style="font-size:20pt">${esc(k[0])}</div><div class="stat-l">${esc(k[1])}</div></div>`).join('')}</div>
    </div>
  </div>`, `Concept ${c.num} — ${esc(c.nom)}`, 'Mise en œuvre');

const conceptPages = (c) => c.deep
  ? [cOuverture(c), cIdee(c), cArchi(c), cEcrans(c), cEcrans2(c, [4, 5], 'Écrans II', 'Écrans — bureau, suite'), cPhones(c), cAnatomie(c), cIphoneNotes(c), cTech(c)]
  : [cOuverture(c), cIdee(c), cArchi(c), cEcrans(c), cMobile(c)];
...
for (const c of CONCEPTS) body.push(...conceptPages(c));
...
for (const c of CONCEPTS) { tocRows.push([`p. ${String(n).padStart(2, '0')}`, `${c.num} — ${c.nom}`, c.pitchCourt]); n += c.deep ? 9 : 5; }
...
for (const c of CONCEPTS) bodyHtml.push(...conceptPages(c));
...
fs.writeFileSync(`${BASE}/dossier5.html`, html);

const browser = await chromium.launch();
const page = await browser.newPage();
await page.goto(`file://${BASE}/dossier5.html`, { waitUntil: 'load' });
...
await page.pdf({ path: `${BASE}/MauvaisGrain-veille-montage.pdf`, width: '297mm', height: '210mm', printBackground: true, margin: { top: '0', right: '0', bottom: '0', left: '0' } });
await browser.close();
