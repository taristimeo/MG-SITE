...
export function ProjectStory({ sections }: { sections: StorySection[] }) {
  return (
    <div className="mx-auto flex max-w-[680px] flex-col gap-20 text-center sm:gap-28">
      {sections.map((s, i) => (
...
function StoryBlock({
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden
        className="story-ghost pointer-events-none absolute left-1/2 top-1/2 -z-10 -translate-x-1/2 -translate-y-1/2 select-none font-wide leading-none text-[clamp(7rem,18vw,13rem)] text-[rgba(185,110,77,0.07)]"
      >
        {num}
      </span>
      ...
      <span
        aria-hidden
        className="story-rule mx-auto mt-4 block h-px w-12 origin-center bg-[var(--color-terra)]"
      />

      <h2 className="story-title font-wide mt-5 text-2xl text-[var(--color-bone)] sm:text-3xl">
        {label}
      </h2>
      <p className="story-body font-sans mx-auto mt-5 text-[1.05rem] leading-[1.7] text-[var(--color-bone-dim)]">
        {body}
      </p>
    </div>
  );
